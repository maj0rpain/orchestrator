...
plan. An optional `--issue N` adopts an already-open, already-triaged issue as
the flow's spec instead of letting the spec phase publish a new one - pass it
through to `"$ORCH" init` as-is.
