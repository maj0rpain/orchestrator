...
reviewer agents, not `code-review`'s sub-agents. And by ADR-0017: the driving
session no longer keeps the bookkeeping in its context or does the fixing and
filing - fresh fixer and closer agents do, reading the records. One session
per loop, and the reviewers' independence from its reasoning, still stand.
